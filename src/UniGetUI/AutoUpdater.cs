    ...
    )
    {
        ...
        try
        {
            ...
            if (updateCandidate.IsUpgradable)
            {
                ...
                {
                    UpdaterDownloadEngine.ClearFailureState(failureStatePath, message => LogUpdateWarn(message));
                ...
                if (!ManualCheck && !Verbose && UpdaterDownloadEngine.IsFailureBackoffActive(
                    ...
                    out UpdaterDownloadFailureState? failure, message => LogUpdateWarn(message)))
                {
                ...
                {
                    UpdaterDownloadEngine.RecordFailure(failureStatePath, identity, "download", DateTime.UtcNow, message => LogUpdateWarn(message));
                    throw;
                }
                ...
                {
                    UpdaterDownloadEngine.PromotePartialDownload(InstallerPath, message => LogUpdateWarn(message));
                    UpdaterDownloadEngine.ClearFailureState(failureStatePath, message => LogUpdateWarn(message));
                ...
                UpdaterDownloadEngine.RecordFailure(failureStatePath, identity, "validation", DateTime.UtcNow, message => LogUpdateWarn(message));
                UpdaterDownloadEngine.DeletePartialDownload(InstallerPath, message => LogUpdateWarn(message));
                ShowMessage_ThreadSafe(
...
#pragma warning restore SYSLIB0057
    ...
    )
    {
        ...
        using (HttpClient client = new(CreateHttpClientHandler(updaterOverrides)))
        {
            client.Timeout = TimeSpan.FromSeconds(600);
            client.DefaultRequestHeaders.UserAgent.ParseAdd(CoreData.UserAgentString);
            var result = await UpdaterDownloadEngine.DownloadInstallerPartAsync(client, identity, installerLocation, message => LogUpdateWarn(message));
